...
export default async function SinglePageRecipe({ params }: Props) {
  ...
  return (
    <main className="flex justify-between gap-16 my-8 p-4 max-w-screen-xl h-screen w-full mx-auto">
      <section className="relative w-1/2 h-96 mb-2">
        <Image
          src={recipe.image}
          alt={recipe.title}
          fill
      ...
      <section className="flex flex-col gap-4 justify-start items-start w-1/2">
        ...
        <div className=" border-gray-500 min-w-full max-h-fit p-4 bg-gray-200 rounded-xl"></div>
      </section>
    </main>
  );
